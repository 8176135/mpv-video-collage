...
use winit::platform::unix::{WindowExtUnix, WindowBuilderExtUnix};
...
impl RandomPathGenerator for RemotePath {
    fn get_random(&self) -> String {
        ...
        let res = self.agent.post("https://lan.8176135.xyz:8000/api/random-temp/")
            .send_json(serde_json::to_value(Configuration::default()).unwrap()).unwrap();
...
fn main() {
    ...
    let agent = ureq::AgentBuilder::new()
        .build();
    let res = agent.post(
        "https://lan.8176135.xyz:8000/login")
        .send_form(&[("password", &std::env::args().skip(1).next().expect("No password passed in"))])
        .unwrap();

    dbg!(res);
    let path_collection = RemotePath {
        agent
    };
    ...
    spawn_mpv_window(path_collection);
}

fn spawn_mpv_window(path_collection: impl RandomPathGenerator + 'static) {
...
fn append_random_playlist(mpv: &Mpv, path_collection: &impl RandomPathGenerator) {
    ...
    mpv
        .playlist_load_files(&[(&path_to_play,
                                FileState::AppendPlay,
                                Some(&format!("length={},start={}%", length_in_seconds, start_pos)),
        )])
        .unwrap();
}
...
fn generate_sub_window(
    ...
    let id = dbg!(child_window.xlib_window()).unwrap() as i64;
